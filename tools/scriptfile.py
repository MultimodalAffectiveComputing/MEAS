#generate scriptfile for Chinese language
...
def script_file_generate_id(id):
    time_s = time.time()
    # #path1
    # isExist=os.path.exists('/media/ExtHDD02/zyq/news_dataset2/script/{0}_script.csv'.format(id))
    #path2
    isExist=os.path.exists('/media/ExtHDD02/zyq/news_dataset2_400_600/script/{0}_script.csv'.format(id))
    if not isExist:
        # #path1
        # with open('/media/ExtHDD02/zyq/news_dataset2/script/{0}_script.csv'.format(id), 'w+', errors='ignore', encoding='utf-8') as f1:
        # path2
        with open('/media/ExtHDD02/zyq/news_dataset2_400_600/script/{0}_script.csv'.format(id), 'w+', errors='ignore',
            ...
            #audio_path2
            sound=AudioSegment.from_mp3('/media/ExtHDD02/zyq/news_dataset2_400_600/audio/{0}.wav'.format(id))
            loundness=sound.dBFS
            print(loundness)

            se_list=detect_silence(sound,min_silence_len=800,silence_thresh=loundness-5)
            start=0
            l=len(se_list)
            for i,se in enumerate(se_list):
                ...
                forced_decoder_ids = processor.get_decoder_prompt_ids(language="en", task="transcribe")#(language="zh", task="translate")
...
def script_file_generate_title(title,basepath):
    time_s = time.time()
    isExist=os.path.exists(f'{basepath}/script/{title}_script.csv')
    if not isExist:
        with open(f'{basepath}/script/{title}_script.csv', 'w+', errors='ignore',
                  encoding='utf-8') as f1:

            writer1=csv.writer(f1,delimiter=';')
            writer1.writerow(['starttime','endtime','text'])
            processor = WhisperProcessor.from_pretrained('/home/pretrained/whisper-large-v2')
            model = WhisperForConditionalGeneration.from_pretrained('/home/pretrained/whisper-large-v2')
            os.mkdir('audio_{0}'.format(id))
            sound=AudioSegment.from_mp3(f'{basepath}/audio/{title}.wav')
            loundness=sound.dBFS
            print(loundness)

            se_list=detect_silence(sound,min_silence_len=450,silence_thresh=loundness-5)#EN 750 CH 800
            start=0
            l=len(se_list)
            for i,se in enumerate(se_list):
                middle=(se[0]+se[1])/2
...
if __name__ == '__main__':
    ...
    script_file_generate_title('Apple_personal_2','/home/system/news_analysis_system_zyq_5_31/news_analysis_system_zyq/test')