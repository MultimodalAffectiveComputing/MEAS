#for batch videos and audios ,no cut
import os
import glob
import torch
...
def retrieve_audio(signal, sr):
    audio_segment = signal
    return audio_segment, sr

def retrieve_video(frames, fps):
    images = frames
    return images, fps
...
def get_dataset_iemocap(base_path:str,id:int,data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    ...
    #text choose
    #text_fname=f'{base_path}/text/en_text.csv'
    #texts=data['en_text'].values
    # data=pd.read_csv(text_fname,delimiter=';')
    # ids=data['id'].values
    # text=''
    # for i in range(len(ids)):
    #     if ids[i]==id:
    #         text=texts[i]
    text_fname=f'{base_path}/translation/{id}_translation.txt'
    text=open(text_fname).readlines()[0]
...
class IEMOCAP(Dataset):
    def __init__(self, images,signal,main_folder: str, texts: List[List[int]],
                 label_annotations: List[str], img_interval: int, metadata: Dict):
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        audio_segment, sr = retrieve_audio(self.signal,self.sr)
        img_segment, fps = retrieve_video(self.images,self.fps)
        ...
        return  sampledImgs, specgrams, self.texts
