...
def get_dataset_iemocap(filename:str, base_path:str,data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    #meta = all_metas
    ...
    # test
    # avi_fname ='video/cry.avi'
    # wav_fname='video/cry.wav'
    avi_fname = f'{base_path}/{filename}.avi'
    wav_fname = f'{base_path}/{filename}.wav'
    script_fname = f'{base_path}/{filename}_translation.txt'

    signal, sr = torchaudio.load(wav_fname)

    images, fps = read_video(avi_fname)

    text = open(script_fname).readline()
