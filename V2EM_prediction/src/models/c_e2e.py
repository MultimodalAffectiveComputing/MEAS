...
from src.models.vgg_block import RepVGGBlock
from src.models.nestnet.nest import Nest
...
class MME2E(nn.Module):
    def __init__(self, args, device):
        ...
        self.V = nn.Sequential(
            ...
            RepVGGBlock(in_channels=64,out_channels=64),
            RepVGGBlock(in_channels=64,out_channels=64),
            RepVGGBlock(in_channels=64,out_channels=128),

            nn.MaxPool2d(kernel_size=2, stride=2),
            RepVGGBlock(in_channels=128,out_channels=256),

            nn.MaxPool2d(kernel_size=2, stride=2),
            RepVGGBlock(in_channels=256,out_channels=512),

            nn.MaxPool2d(kernel_size=2, stride=2),
        )

        self.A = nn.Sequential(
            Nest(img_size=32,in_chans=4,patch_size=1)
        )
    ...
    def forward(self, imgs, imgs_lens, specs, spec_lens, text):
        ...
        if 't' in self.mod:
            ...
            #text output write
            text_logits=self.t_out(text_cls)
        ...
        if 'v' in self.mod:
            ...
            for i, face in enumerate(faces):
                if face is None:
                    ...
                    faces[i] = center
            face=faces[0].permute(1,2,0)
            cv2.imwrite('face.png',face.numpy())

            #如果有问题，取消注释
            face_all=torch.Tensor(3,48,48)
            for face in faces:
                if face.shape==face_all.shape:
                    face_all=face
            for i in range(len(faces)):
                if faces[i].shape!=face_all.shape:
                    faces[i]=face_all
        ...
        if 'a' in self.mod:
            ...
            # audio output write
            audio_logits=self.a_out(specs)


        if len(self.mod) == 1:
            return all_logits[0]

        return self.weighted_fusion(torch.stack(all_logits, dim=-1)).squeeze(-1),text_logits,video_logits,audio_logits
