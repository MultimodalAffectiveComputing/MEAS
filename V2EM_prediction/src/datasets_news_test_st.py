...
def parse_evaluation_transcript(script_fn):
    metadata = {}
    data = pd.read_csv(script_fn, encoding='utf-8', sep=';')
    sts=data['starttime'].values
    ets=data['endtime'].values
    texts=data['text'].values
    for i in range(len(sts)):
        metadata[i]={'start_time': sts[i], 'end_time': ets[i]}
    ...
    #         metadata[uttr_id] = {'start_time': start_time, 'end_time': end_time}
    #
    # # Parse Transcript
    # trans = []
    # for line in transcript_lines:
...
def crop(imgs, target_size=224):
    # imgs.shape = (2280, 640, 360, 3)
    #hand_crop
    _, h, w, _ = imgs.shape
    offset_h = 150
    offset_w = (w - target_size) // 2
    imgs = imgs[:, (200-target_size//2):(200+target_size//2), offset_w:-offset_w, :]
    return imgs
...
def get_dataset_iemocap(file_name:str,data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    ...
    if file_name=='temp':
        print('find temp')
        avi_fname = 'V2EM_prediction/temp/video_processed.avi'
        wav_fname = 'V2EM_prediction/temp/audio.wav'
        script_fname = 'V2EM_prediction/temp/script.csv'
        # wav_fname = '/home/system/news_analysis_system_zyq_5_31/news_analysis_system_zyq/temp/audio.wav'
        # script_fname = '/home/system/news_analysis_system_zyq_5_31/news_analysis_system_zyq/temp/script.csv'
        # avi_fname = '/home/system/news_analysis_system_zyq_5_31/news_analysis_system_zyq/temp/video_processed.avi'
    else:
        print('not temp')
        avi_fname = f'test/video/{file_name}.avi'
        wav_fname = f'test/audio/{file_name}.wav'
        script_fname = f'test/script/{file_name}_script.csv'
    ...
    metas = parse_evaluation_transcript(script_fname)
    #
...
class IEMOCAP(Dataset):
    def __init__(self, meta,signal,images,main_folder: str, utterance_ids: List[str], texts: List[List[int]],
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        metadata=self.all_metas[uttrId]
        # use_left = self.use_left(uttrId)
        suffix = 'L'

        audio_segment, sr = retrieve_audio(self.signal,metadata['sr'],metadata['start_time'], metadata['end_time'])
        img_segment, fps = retrieve_video(self.images,metadata['fps'],metadata['start_time'], metadata['end_time'])
       # img_segment_L, img_segment_R = img_segment[:,:,:img_segment.shape[2] // 2,:], img_segment[:,:,img_segment.shape[2] // 2:,:]
        #img_segment_L = crop(img_segment)
        nums=len(img_segment)
        step = int(self.img_interval / 1000 * fps)+1
        if suffix == 'L':
            sampledImgs=np.array([np.float32(img_segment[i]) for i in list(range(0,nums,step)) ])
            waveform=audio_segment[0] #left audio channel

        waveform=waveform.unsqueeze(0)#add one dim
        specgram = torchaudio.transforms.MelSpectrogram(sample_rate=sr, win_length=int(400))(waveform).unsqueeze(0)
        specgrams = self.cutSpecToPieces(specgram)

        return  uttrId, sampledImgs, specgrams, self.texts[ind]
...
def collate_fn(batch):
    ...
    for dp in batch:
        utteranceId, sampledImgs, specgram, text = dp
