#input for a video and audio,no cut
...
#avi_fname =f'{base_path}/47.avi'
#wav_fname=f'{base_path}/5.wav'
script_fname=f'{base_path}/5_translation.txt'
...
class IEMOCAP(Dataset):
    def __init__(self, main_folder: str, texts: List[List[int]],
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        suffix = 'L'
        audio_segment, sr = retrieve_audio(signal,self.sr)
        img_segment, fps = retrieve_video(images,self.fps)
