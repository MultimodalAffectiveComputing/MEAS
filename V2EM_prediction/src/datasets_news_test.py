...
base_path='../videonews'#back directory


#eval_fname=f'{base_path}/Ses01F_impro02_label.txt'

#eval_lines = open(eval_fname).readlines()
avi_fname =f'{base_path}/5_2.avi'
wav_fname=f'{base_path}/5.wav'
script_fname=f'{base_path}/5_script.csv'
...
class IEMOCAP(Dataset):
    def __init__(self, main_folder: str, utterance_ids: List[str], texts: List[List[int]],
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        audio_segment, sr = retrieve_audio(signal,metadata['sr'],metadata['start_time'], metadata['end_time'])
        img_segment, fps = retrieve_video(images,metadata['fps'],metadata['start_time'], metadata['end_time'])
